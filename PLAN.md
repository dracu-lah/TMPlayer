...
| 4 | Streaming playback while downloading, **robust seek fwd/back**, embedded subtitles, audio-track switching, resume position | Media3 ExoPlayer + custom `TdDataSource` + `media3-ui-leanback` player UI |
...
- "Continue watching" home row (v2 — per-file auto-resume IS in v1)
...
- On scan: full-screen loader "Confirming…". If account has 2FA: password screen — one large password field, TV keyboard, big "Unlock" button, password hint shown (TDLib provides it).
...
- Focused row: subtle scale (1.02–1.05) + high-contrast border — tv-material Card defaults, not custom.
- Top-right small "Settings" affordance reachable by D-pad up.

### 3.3 Media Grid
- Grid of poster cards, **3 columns** (big cards ≈ 400 px wide on 1080p — laid-back, oversized on purpose). Card = video thumbnail (TDLib minithumbnail instantly, real thumbnail when fetched), duration badge, one-line filename, size + date caption line.
...
### 3.4 Player
- **No custom player UI.** `PlaybackSupportFragment` + `LeanbackPlayerAdapter` from `androidx.media3:media3-ui-leanback` — a complete, D-pad-native TV transport UI: play/pause, seek bar, fast-forward/rewind with press-and-hold acceleration, title row. We only configure it.
...
## 6. Streaming & seeking design (the critical 400 lines)

**Why this is the make-or-break piece:** naive implementations download from byte 0 only, so seeking forward = wait for the whole file. TDLib natively supports downloading **from any offset** (`downloadFile(fileId, priority, offset, limit, synchronous=false)`) and reports how many bytes are available from that offset (`updateFile → local.downloadOffset / downloadedPrefixSize`, plus `getFileDownloadedPrefixSize`). The official Telegram app streams exactly this way (tdlib/td#1498).
...
### Seek handling (the part the user flagged: *must not be a problem*)
...
### Reuse, not invention — updated after verification (2026-08-07)
...
Mechanisms (all TDLib built-ins — zero custom file bookkeeping):
...
|---|---|---|
| Language / build | Kotlin 2.x, AGP 8.x, single module, `minSdk 26 / compileSdk+targetSdk latest` | — |
| **TDLib** | **DECIDED: `dev.g000sha256:tdl-coroutines:9.0.0`** — verified on Maven Central: Apache-2.0, TDLib 1.8.66, AAR bundles `libtdjsonjava.so` for armeabi-v7a / arm64-v8a / x86 / x86_64, ~1010 typed suspend request methods + ~184 update `Flow`s | Best-in-class fit; x86_64 native included → emulator works. (TGX bundle ruled out — its README forbids use outside Telegram X; tdlight's Android natives unclear.) **We never compile TDLib ourselves.** |
...
| DI / other | none — manual singletons; kotlinx-coroutines | minimal-work rule |

### Reference implementations (read before coding the matching milestone)
| Reference | Use for |
|---|---|
| [keygenqt/android-tvgram](https://github.com/keygenqt/android-tvgram) (Apache-2.0) | TDLib-on-TV wiring, playback approach — closest OSS cousin, safe to copy |
...
| Emulator lacks matching ABI for TDLib | Prefer artifact with x86_64; else debug-on-device only (wireless ADB) — acceptable. |

## 11. Needed from you (before M1)
1. ~~**`api_id` + `api_hash`**~~ ✅ received — stored in untracked `local.properties`.
2. **Stick variant** — Settings → Device Preferences → About (model MDZ-24-AA = 2020 1080p, MDZ-27-AA = 4K). Design already covers both; this only tunes defaults. *(Still open — user will check.)*
3. **Wireless ADB enabled** on the stick when we reach on-device testing (About → click "Build" 7× → Developer options → USB/Network debugging ON). I'll provide exact steps in INSTALL.md at M0.
